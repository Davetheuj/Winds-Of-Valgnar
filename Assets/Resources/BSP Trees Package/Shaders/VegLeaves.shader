...
Shader "Custom/HDRPVegLeaves"
{
	Properties
	{
		...
		_Flutter("Flutter", Range( 0 , 1)) = 0.5
		_FlutterStrength("Flutter Strength", Range( 0 , 1)) = 0.5
		[ASEEnd]NoiseTextureFloat1("Noise Texture", 2D) = "white" {}
	...
	SubShader
	{
		...
		Pass
		{
			...
			PackedVaryingsMeshToPS VertexFunction(AttributesMesh inputMesh )
			{
				...
				#endif
				float3 vertexValue = staticSwitch176.rgb;
						...
						)
			{
				...
				float cos184 = cos( ( tex2D( NoiseTextureFloat1, Noise166.rg ) * _Flutter * _FlutterStrength ).r );
				float sin184 = sin( ( tex2D( NoiseTextureFloat1, Noise166.rg ) * _Flutter * _FlutterStrength ).r );
